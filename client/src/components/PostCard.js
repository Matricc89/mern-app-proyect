  ...
  const handleDelete = (_id) => {
    toast((t) => (
      <div >
        ...
        <div>
          <button className="bg-red-500 hover:bg-red-400 px-3 py-2 text-s text-white rounded-sm mx-2" onClick={() => {
            deletePost(_id)
            toast.dismiss(t._id)
          }}>Delete</button>
          <button className="bg-slate-400 hover:bg-slate-500 px-3 py-2 text-white rounded-sm mx-2" onClick={() => toast.dismiss(t._id)}>
            Cancel
          </button>
        </div>
      </div>
